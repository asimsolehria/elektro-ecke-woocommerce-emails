...
/**
 * Customer stock notification &mdash; verified email
 *
 * New in WooCommerce (back-in-stock notifications, double opt-in step):
 * sent once a customer has confirmed they'd like to be notified when a
 * specific out-of-stock product is back.
 *
...
<tr>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<td class="resize" style=" font-family: 'Outfit', sans-serif;font-size:31px;text-align:center;color:#1F2548;font-weight:700;line-height: 45px;">
								<?php esc_html_e('You\'re all set', 'e-mail-strings'); ?> </td>
						</tr>
						<tr>
						...
						<tr>
							...
							<?php
								if ($product_name) {
									printf(esc_html__('We\'ll email you the moment %s is back in stock.', 'e-mail-strings'), '<strong>' . esc_html($product_name) . '</strong>'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								} else {
									esc_html_e('We\'ll email you the moment this item is back in stock.', 'e-mail-strings');
								}
							?> </td>
						</tr>
						<tr>
    ...
    width: auto' href="<?php echo esc_url($product_url ? $product_url : wc_get_page_permalink('shop')); ?>"> <?php esc_html_e('Keep Browsing', 'e-mail-strings') ?>
				</a>
			</div>
		</td>
	</tr>
